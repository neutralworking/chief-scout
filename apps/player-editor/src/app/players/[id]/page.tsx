...
async function fetchPlayer(id: string): Promise<IntelligenceCard | null> {
...
function DimensionBar({
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="text-[10px] font-bold w-3 text-[var(--accent-personality)]">
        {left}
      </span>
      ...
      <span className="text-[10px] font-bold w-3 text-[var(--text-muted)]">
        {right}
      </span>
      <span className="text-xs font-mono w-6 text-right text-[var(--text-secondary)]">
        {value}
      </span>
    </div>
  );
...
export default async function PlayerDetailPage({
  ...
  const posColor =
    POSITION_COLORS[player.position ?? ""] ?? "bg-zinc-700/60";
  const pursuitColor =
    PURSUIT_COLORS[player.pursuit_status ?? ""] ?? "";
    ...
    <div className="max-w-4xl">
      ...
      <div className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-lg p-6 mb-4">
        <div className="flex items-start justify-between">
          ...
          <div className="flex items-center gap-2">
            <span
              className={`text-[10px] font-bold tracking-wider px-2 py-1 rounded ${posColor} text-white`}
            >
              {player.position ?? "–"}
            </span>
            {player.pursuit_status && (
              <span
                className={`text-[10px] font-semibold tracking-wide px-2 py-1 rounded ${pursuitColor}`}
              >
                {player.pursuit_status}
              </span>
            )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        ...
        <div className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-lg p-6">
          ...
          {player.archetype ? (
            <div>
              <div className="text-xl font-semibold text-[var(--accent-tactical)] mb-1">
                {player.archetype}
              </div>
              {player.model_id && (
              ...
              {player.blueprint && (
                <div className="mt-4">
                  <h4 className="text-[10px] font-semibold tracking-widest uppercase text-[var(--text-muted)] mb-2">
                    Blueprint
                  </h4>
        ...
        <div className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-lg p-6 mb-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            {player.market_value_tier && (
              <div>
                <span className="text-[10px] text-[var(--text-muted)] uppercase tracking-wide block">
                  MVT
                </span>
                <span className="font-mono font-bold">
                  {player.market_value_tier}
            ...
            {player.true_mvt && (
              <div>
                ...
                <span className="font-mono font-bold">{player.true_mvt}</span>
              </div>
            )}
