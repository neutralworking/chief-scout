...
export default function PersonalityReassessmentPage() {
...
function AssessmentPanel({
  ...
  useEffect(() => {
    ...
    setTf(player.tf);
    setJp(player.jp);
    setComp(player.competitiveness ?? 5);
    setCoach(player.coachability ?? 5);
    setError(null);
  ...
  return (
    <div className="glass rounded-xl p-4 space-y-4 max-h-[calc(100vh-120px)] overflow-y-auto">
      ...
      <div className="flex flex-wrap gap-2 text-[10px]">
        {player.trajectory && (
          <span className="px-2 py-0.5 rounded bg-[var(--bg-elevated)] text-[var(--text-secondary)]">
            {player.trajectory}
          </span>
        )}
        {player.loyalty_score != null && (
          <span className="px-2 py-0.5 rounded bg-[var(--bg-elevated)] text-[var(--text-secondary)]">
            loyalty: {player.loyalty_score}/20
          </span>
        )}
        {player.clubs_count != null && (
          <span className="px-2 py-0.5 rounded bg-[var(--bg-elevated)] text-[var(--text-muted)]">
            {player.clubs_count} clubs / {player.career_years?.toFixed(0) ?? "?"}yr
          </span>
        )}
        {player.tags.slice(0, 8).map(t => (
          <span key={t} className="px-1.5 py-0.5 rounded bg-purple-500/15 text-purple-400 text-[9px]">{t}</span>
        ))}
      </div>
      ...
      <div className="text-center py-2">
        ...
        {info && (
          <p className="text-xs font-semibold text-[var(--text-primary)] mt-1">{info.name}</p>
        )}
        {info && (
          <p className="text-[10px] text-[var(--text-secondary)] mt-0.5">{info.oneLiner}</p>
        )}
        {changed && (
          <p className="text-[9px] text-amber-400 mt-1">
            Changed from {originalCode} ({PERSONALITY_NAMES[originalCode]?.name ?? "?"})
          </p>
        )}
      </div>
      ...
      {/* Traits: competitiveness + coachability */}
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          ...
          <input
            ...
            max={10}
            value={coach}
            onChange={e => setCoach(Number(e.target.value))}
            className="w-full h-2 accent-[var(--color-accent-personality)]"
          />
