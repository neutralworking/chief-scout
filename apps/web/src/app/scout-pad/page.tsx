...
export default function ScoutPadPage() {
  ...
  const handleEditKeyDown = (e: React.KeyboardEvent, playerId: number, field: string) => {
    if (e.key === "Enter") {
      e.preventDefault();
      saveField(playerId, field, editValue);
    }
    if (e.key === "Escape") {
      setEditingCell(null);
    }
  ...
  return (
    <div>
      <h1 className="text-lg font-bold tracking-tight mb-0.5">Scout Pad</h1>
      ...
      <div className="flex flex-wrap gap-2 mb-3">
        ...
        >
          {PURSUIT_STATUSES.map((s) => (
            <option key={s} value={s}>{s === "All" ? "All Pursuit" : s}</option>
          ))}
        </select>
      </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-1.5 px-3">
                        ...
                        {p.club && (
                          <span className="text-[9px] text-[var(--text-muted)] ml-1.5">{p.club}</span>
                        )}
                      </td>
                        ...
                        {editingCell?.id === p.person_id && editingCell?.field === "position" ? (
                          <select
                            ...
                            onChange={(e) => {
                              ...
                              saveField(p.person_id, "position", e.target.value);
                            }}
                      ...
                      <td className="py-1.5 px-2 text-[10px] text-[var(--text-secondary)]">
                        {p.best_role ?? <span className="text-[var(--text-muted)]">–</span>}
                      </td>

                      {/* Role Score — read only */}
