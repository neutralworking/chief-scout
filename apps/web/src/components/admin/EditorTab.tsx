    ...
    <div>
      ...
      <div className="flex gap-2 mb-4">
        <input
          ...
          className="flex-1 px-3 py-2 rounded-lg bg-[var(--bg-elevated)] border border-[var(--border-subtle)] text-sm text-[var(--text-primary)] placeholder:text-[var(--text-muted)] focus:outline-none focus:border-[var(--color-accent-tactical)] transition-colors"
        />
        <button
          onClick={search}
          disabled={(query.length < 2 && !positionFilter && !missingFilter) || loading}
          className="px-4 py-2 rounded-lg bg-[var(--color-accent-tactical)] text-[var(--bg-base)] text-sm font-semibold hover:opacity-90 disabled:opacity-40 transition-opacity"
        >
          {loading ? "..." : "Search"}
        </button>
      </div>
      ...
      {!loading && searched && results.length === 0 && (
        <div className="text-center py-8 text-[var(--text-muted)] text-sm">No players found.</div>
      )}
          ...
          {results.map((p) => {
            ...
            return (
              <Link
                ...
                className="flex items-center gap-3 px-4 py-3 rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-surface-solid)] hover:border-[var(--color-accent-tactical)]/50 hover:bg-[var(--bg-elevated)]/50 transition-all group"
              >
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="font-semibold text-sm group-hover:text-[var(--color-accent-tactical)] transition-colors">
                      {p.name}
                    </span>
                    {p.position && (
                  ...
                  <div className="text-xs text-[var(--text-muted)] mt-0.5">
                    ...
                    {p.pursuit_status && <span className="ml-2 text-[var(--color-accent-tactical)]">{p.pursuit_status}</span>}
                  </div>
                </div>
                ...
                <div className="flex items-center gap-1.5 shrink-0" title={comp.missing.length > 0 ? `Missing: ${comp.missing.join(", ")}` : "Complete"}>
                  <div className="flex gap-0.5">
                    {Array.from({ length: comp.total }, (_, i) => (
                      <div
                        key={i}
                        className={`w-1.5 h-3 rounded-sm ${i < comp.filled ? "bg-[var(--color-accent-tactical)]" : "bg-[var(--border-subtle)]"}`}
                      />
                    ))}
                  </div>
                ...
                <svg className="w-4 h-4 text-[var(--text-muted)] group-hover:text-[var(--color-accent-tactical)] transition-colors shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
