...
parser.add_argument("--no-calibration", action="store_true",
                    help="Skip DoF calibration corrections")
...
def main():
    ...
    # Load DoF calibration corrections
    corrections = None
    if not args.no_calibration:
        try:
            ...
            if corrections.n_assessments > 0:
                print(f"\n  Loaded DoF calibration from {corrections.n_assessments} assessments")
                print(f"  Global bias: {corrections.global_bias:.3f}x")
            else:
                print("\n  No DoF assessments found — running uncalibrated")
        except Exception as e:
            print(f"\n  Warning: Could not load DoF calibration: {e}")
    ...
    for row in rows:
        ...
        try:
            ...
            response = run_valuation(profile, context, mode, corrections=corrections)
